...
public class PietsGrafieken {
    ...
    public static class Builder {
        ...
        private List<FunctionNameColor> functions = new ArrayList<>();
        ...
        public Builder withFunctionNameColor(Function<Double, Double> f, String name, Color c) {
            functions.add(new FunctionNameColor(f, name, c));
            return this;
        }
    ...
    private static class Grafiek extends JPanel {
        ...
        // instance members
        private final List<FunctionNameColor> functions;
        private final Map<FunctionNameColor, Path2D> paths = new HashMap<>();
...
//        List<JLabel> axeslabels = new ArrayList<>();
        ...
        private Grafiek(double xmin, double xmax, double ymin, double ymax,
                List<FunctionNameColor> list, int panelWidth) throws NoninvertibleTransformException {
            //------------------------------------------------
            // null-layout
            this.setLayout(null);
            // ratio height / width
        ...
        private void createFunctionPaths() {
            for (var fnc: functions) {
                var path = new Path2D.Double();
                double x = 0;
                try {
                   for (int i = 0; i < viewWidth; i++) {
                       x = (topLeft.getX() * (viewWidth - i) + bottomRight.getX() * i) / viewWidth;
                       var y = fnc.function().apply(x);
                   ...
                   paths.put(fnc, path);
                    ...
                    System.out.println("Error in function for x = " + x);
                }
            }
        }
        ...
        private void createAxesAndLabels() {
            ...
            for (var e: functions) {
                var len = e.name().length() * 8;
                var label = new JLabel(e.name());
                label.setForeground(e.color());
...
//            }
    ...
    public final static class FourierSeries {
        ...
        public static interface Mouski extends BiFunction<Double, Integer, Double> {}
    }
}

//==============================================================================
record FunctionNameColor(Function<Double, Double> function, String name, Color color) {
    
    public Path2D createPath(double xmin, double xmax, int nrOfPoints) {
        var result = new Path2D.Double();
        for (int i = 0; i <= nrOfPoints; i++) {
            var d = xmin * (1 - i) + xmax * i;
            var val = function.apply(d);
            if (i == 0) result.moveTo(d, val);
            else result.lineTo(d, val);
        }
        return result;
    }
}
